...
#from checkMiSeq.py
#v2 make sure all references are in legend, even if 0 reads map to them
...
#function to add references to data frame that have 0 reads mapping to it
get_zero_refs <- function(sample) {
  for (r in refs) {
    #assign 0 to reference genomes that were not hit
    if (r %in% sample$taxon) {
    }
    else {
      row <- c(sample$runname[1], sample$sample[1], sample$snum[1], r, 0, sample$total[1])
...
#list of references used in read mapping
refs <- c('phiX174','Ecoli','Pdenitrificans','Pacnes','hg38','HCV','HIV1','GBvirusC','GBvirusB','HumanPegivirus2', 'HBV','MMLV','not_referenced')
...
by_sample <- split(df, df$sample)     #split df by sample, returns list of data frames
by_sample_getzeros <- lapply(by_sample, get_zero_refs)    #apply function to each item of list
df_4plot <- ldply(by_sample_getzeros, data.frame)       #merge list of data frames
...
df_4plot$taxon_order <- factor(df_4plot$taxon, levels = c('phiX174','Ecoli','Pdenitrificans','Pacnes','hg38','HIV1','HCV','GBvirusC','GBvirusB','HumanPegivirus2','HBV','MMLV','not_referenced'))
