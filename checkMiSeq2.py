...
def do_map(fastq1, fastq2, refpath, bowtie_threads, min_match_len, min_mapq, min_score):
    ...
    with p.stdout:
        for line in p.stdout:
            ...
            # ignore reads that did not map to a reference
            if rname == '*':
                n_unmapped += 1
                continue
    ...
    return counts, n_unmapped, n_short, n_mapq, n_hybrid, n_low
...
def main():
    ...
    for path in paths:
        ...
        for f1 in files:
            ...
            counts, n_unmapped, n_short, n_mapq, n_hybrid, n_low = do_map(f1, f2, args.x, args.p, args.minlen, args.minq, args.mins)
            n_discard = n_unmapped + n_short + n_mapq + n_hybrid + n_low
